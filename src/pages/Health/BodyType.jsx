    ...
    <div>
            ...
            <div className={"w-100 h-75 p-4 bg-secondary-subtle border border-3 border-secondary"}>
              {result ? (
                <>
                  ...
                  {Object.values(result).map((value, index) => (<p key={index}>{value}</p>))}
                </>
              ) : ""
              }
            </div>
      ...
      <div>
        <div className={"mb-5"}>
          ...
          <p>Тип телосложения (соматотип) ─ это пропорции частей тела человека в сочетании с индивидуальными
            особенностями
            развития костной, жировой и мышечной тканей.
            Размеры и формы тела каждого человека могут существенно различаться у разных людей и в значительной степени
            генетически запрограммированы. Эта программа реализуется в ходе естественных морфологических,
            физиологических
            и биохимических трансформаций организма в течение всей жизни человека.
          </p>
        </div>
        <div className={"mb-5"}>
          <h3>Индекс Пинье</h3>
        ...
        <div className={"mb-5"}>
          <h3>Индекс Рорера</h3>
          <p>Индекс Рорера (коэффициент упитанности), характеризует относительную плотность тела. Был впервые предложен
            в 1921 году в качестве "показателя полноты" швейцарским врачом Фрицем Рорером.</p>
        ...
        <div className={"mb-5"}>
          ...
          <p>Индекс Соловьёва используется для вычисления нормального веса с учетом трех типов телосложения человека.
            ...
            нужно обязательно учитывать, анализируя физическое состояние человека.</p>
          <div>
            <p>Различают 3 типа телосложения, которые непосредственным образом влияют на форму тела человека:</p>
            <ul>
              <li>астеническое телосложение - женщины с окружностью запястья менее 15 см и мужчины с окружностью
                запястья менее 18 см;
              </li>
              <li>нормостеническое телосложение - женщины с окружностью запястья от 15 см до 17 см и мужчины с
                окружностью запястья от 18 см до 20 см;
              </li>
              <li>гиперстеническое телосложение - женщины с окружностью запястья более 17 см и мужчины с окружностью
        ...
        <div className={"mb-5"}>
          ...
          <p>Толщина костей также является важным параметром для врачей-акушеров при диагностике беременных. Поэтому в
            ...
            тоньше). А когда окружность запястья превышает 16 см, то из обхвата большого таза отнимают 10 см.</p>
        </div>
        <div className={"mb-5"}>
          <p>Калькулятор типа телосложения может быть полезен для людей, которые стремятся к улучшению своего здоровья и
            желают следить за своим весом. Данный калькулятор поможет пользователю понять свой тип телосложения и
            определить, какие изменения в образе жизни необходимы для достижения лучших результатов.</p>
        </div>
      </div>
    </div>
  );
