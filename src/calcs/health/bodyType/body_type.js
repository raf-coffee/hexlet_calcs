...
export const bodyType = (height, weight, bust, carpus, sex) => {
  ...
  const resPinie = Object.entries(pin).filter((func) => func[1]() !== false).flat()[1]();
  const resRor = Object.entries(ror).filter((func) => func[1](rorer) !== false).flat()[1](rorer);
  const resSol = Object.entries(Object.entries(soloviev).filter((row) => row.includes(sex))
    .flat()[1]).filter((row) => row[1]()).flat()[1]();
  return {
